...
<div class="container pb-5">
    ...
    <div class="container" style="width: 90%;">
        <form action="{{ route('motor.booking.store', $motor->id) }}" method="POST">
            @csrf
            <div class="row">
                        ...
                        <h4 class="title-booking mb-3">Detail Pemesan</h4>
                            <div class="card p-3 rounded-2">
                                ...
                                <div class="row my-3">
                                    ...
                                    {{-- <input type="hidden" name="grand_total" value="{{ $motor->final_price }}"> --}}
                                    <div class="col-md-2">
                ...
                <div class="col-lg-4">
                    ...
                    <div class="card p-1 rounded-2">
                        <div class="card-body">
                            <div class="d-flex align-items-center justify-content-center">
                                @if ($motor->image)
                                    <img src="{{ asset('img/' . $motor->image) }}" alt="motor Image" class="rounded-2" width="50%">
                                @else
                                    <img src="{{ asset('front-assets/img_blank.jpg') }}" alt="Default Image"/>
                                @endif
                            </div>
                            <div class="my-2">
                                <span class="title-booking-three d-block text-capitalize">{{ $motor->title }}</span>
                            </div>
                            <div class="my-2">
                                <span class="title-booking-three d-block">Disediakan oleh {{ $motor->mitra->name }}</span>
                                {{-- <span class="title-booking-secondary d-block text-capitalize">{{ $motor->title }}</span> --}}
                            </div>
                        </div>
                    </div>
                    <div class="card p-1 my-4">
                        <div class="card-body">
                            ...
                            <hr class="my-3">
                            <div class="form-group form-group-perjalanan my-2">
                                <label for="tgl_pinjam" class="float-start mb-2 title-booking-three">Tanggal Pinjam<span class="text-danger">*</span></label>
                                <input type="text" id="tgl_pinjam_motor" name="tgl_pinjam" class="form-control nav-group-mvp" required>
                            </div>
                            <div class="form-group form-group-perjalanan my-2">
                                <label for="tgl_pengembalian" class="float-start mb-2 title-booking-three">Tanggal Pengembalian<span class="text-danger">*</span></label>
                                <input type="text" id="tgl_pengembalian_motor" name="tgl_pengembalian" class="form-control nav-group-mvp" required>
                            </div>
                            <div class="form-group form-group-perjalanan my-2">
                    ...
                    <div class="card p-1 mt-4">
                        ...
                        <hr class="my-3">
                            <div class="my-2">
                                <h6 class="title-book">1x {{ $motor->title }}</h6>
                            </div>
                            <div class="my-3">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    @else
                                        <span class="title-booking-three d-block">{{ $motor->price_discount }}%</span>
                                    @endif
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    <span class="title-booking-three d-block">Durasi Pinjam</span>
                                    <span class="badge bg-two mb-0 mr-auto" id="durasi-pinjam-badge">0 Hari</span>
                                    <input type="hidden" id="durasi_pinjam" name="durasi_pinjam" value="0">
                                </div>
                            </div>
                            @if ($motor->price_discount === null)
                            ...
                            @else
                                <div class="text-end">
                                    <h5 class="price-sub text-decoration-line-through mb-0">Rp {{ $motor->price }}</h5>
                                </div>
                            @endif
                            <div class="d-flex justify-content-between align-items-center my-2">
                                <h5 class="titles mb-0">Harga Total</h5>
                                <div class="price-end text-end">
                                    <h4 class="mb-0" id="total-price">Rp {{ $motor->final_price }}</h4>
                                    <input type="hidden" name="grand_total" value="{{ $motor->final_price }}">
                                </div>
                            </div>
                        <hr class="my-3">
...
<script>
    ...
    $(function() {
        ...
        $('input[id="tgl_pinjam_motor"], input[id="tgl_pengembalian_motor"]').daterangepicker({
        ...
        // Event listener untuk update input tgl_pinjam_motor dan tgl_pengembalian_motor
        $('input[id="tgl_pinjam_motor"], input[id="tgl_pengembalian_motor"]').on('apply.daterangepicker', function(ev, picker) {
            var startDate = formatDate(picker.startDate.format('YYYY-MM-DD'));
            var endDate = formatDate(picker.endDate.format('YYYY-MM-DD'));
            
            $('#tgl_pinjam_motor').val(startDate);
            $('#tgl_pengembalian_motor').val(endDate);
            ...
            $('#durasi_pinjam').val(nights); // Mengisi input tersembunyi dengan nilai durasi
        });

        // Event listener untuk clear input
        $('input[id="tgl_pinjam_motor"], input[id="tgl_pengembalian_motor"]').on('cancel.daterangepicker', function(ev, picker) {
            $(this).val('');
            $('.badge').text('0 Hari');
            $('#durasi_pinjam').val(0); // Mengisi input tersembunyi dengan nilai 0
        });
    });
