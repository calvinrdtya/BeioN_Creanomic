    ...
    <div class="card p-3">
        <div class="row">
            <div class="col-md-5 d-flex align-items-center justify-content-center">
                @if ($order->mobil->image)
                    <img src="{{ asset('img/' . $order->mobil->image) }}" alt="mobil Image" class="rounded-2" width="60%">
                @else
                    <img src="{{ asset('front-assets/img_blank.jpg') }}" alt="Default Image"/>
                @endif
            </div>
            <div class="col-md-7">
                <h5 class="title-booking">{{ $order->mobil->title }}</h5>
                <div class="row mt-2 mb-2">
                    ...
                    <div class="col-sm-5">
                        <div class="my-1">
                            ...
                            <span class="title-booking-secondary d-block text-capitalize">{{ $order->mobil->kota }}</span>
                        </div>
                        <div class="my-1">
        ...
        <div class="my-2">
            ...
            <div class="row p-2">
                ...
                <div class="col-md-6 justify-content-center my-auto text-end my-1">
                    {{-- <span class="title-booking-three d-block">Rp {{ number_format($order->mobil->final_price, 0, '.', ',') }}</span> --}}
                    <span class="title-booking-three d-block">Rp {{ $order->mobil->final_price }}</span>
                </div>
                <div class="col-md-6 my-1">
                ...
                <div class="col-md-6 justify-content-center my-auto text-end">
                    ...
                    @else
                        <span class="title-booking-three d-block text-decoration-line-through">Rp {{ $order->mobil->price }}</span>
                    @endif
                </div>
                <div class="my-3">
                    <div class="row">
            ...
            @if ($order->payment_status == 1)
                <div class="row">
                    <div class="col-md-6">
                        {{-- <form action="{{ route('user.mobil.delete', ['order_id' => $order->id]) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan order ini?')"> --}}
                            @csrf
                            @method('DELETE')
        ...
        payButton.addEventListener('click', function(e) {
            ...
            snap.pay('{{ $order->snap_token }}', {
                ...
                onSuccess: function(result) {
                    window.location.href = '{{ route('payment.mobil.success', ['mobil_id' => $order->id]) }}';
                },
                // Optional
